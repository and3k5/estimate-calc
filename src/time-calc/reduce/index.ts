...
import type { TimePartItem, TimePartsArray } from "../load-input";
...
function parseTimeOrScope(item: TimePartItem) {
    if (item instanceof Time) return item;
    if (Array.isArray(item)) return reduce(item);
    throw new Error("Unsupported type: " + typeStr(item));
}

function parseAction(item: TimePartItem) {
    if (item instanceof Time) throw new Error("Unsupported type: " + typeStr(item));
    if (Array.isArray(item)) throw new Error("Unsupported type: " + typeStr(item));
    return item;
}
