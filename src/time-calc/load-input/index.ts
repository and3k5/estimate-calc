...
import { type Action, addAction, subAction } from "../actions";
...
export function loadStringParts(str: string) {
    ...
    while (inputArray.length > 0) {
        ...
        if (b == "+") {
            if (i > 0) yieldValue(addAction);
        } else if (b == "-") {
            if (i === 0) yieldValue("0m");
            ...
            if (b.trim() !== "" || hasStringGoing()) appendLastString(b);
        }
...
export function loadInput(setup: TimeSetup, inputValue: InputValue) {
    const str = Array.isArray(inputValue)
        ? inputValue.map((x) => "(" + x + ")").join(" + ")
        : inputValue;
